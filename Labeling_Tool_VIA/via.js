...
var VIA_REGION_SHAPE = { RECT:'rect'}; //@Ira: shapes entfernt
...
function import_annotations_from_json(data) {
  ...
  for (var image_id in d) {
    var filename_from_json = d[image_id]["filename"];
    console.log(image_id+" Filename: "+filename_from_json);
...
//  show_message('Import Summary : [' + region_import_count + '] regions from '+ _via_img_metadata[image_id].filename);
  show_message('Import Summary : [' + region_import_count);
...
function pack_via_metadata(return_type) {
  if( return_type === 'csv' ) {
    ...
    return [JSON.stringify(_via_img_metadata_as_obj, null, 2)];
  }
}
...
function show_img_list() {
  ...
  if(_via_is_loaded_img_list_visible && _via_current_image_loaded) {
    ...
    if ( sel_file.offsetTop > panel_height/2 ) {
      console.log("hello! " + sel_file.offsetTop < panel.scrollTop );//@Andrea
...
_via_reg_canvas.addEventListener('mousedown', function(e) {
  ...
  if ( _via_is_region_selected ) {
    ...
    if ( _via_region_edge[1] > 0 ) {
        console.log("mouse is down and region is selected: ");
...
_via_reg_canvas.addEventListener('mouseup', function(e) {
  ...
  if ( _via_is_user_resizing_region ) {
    ...
    case VIA_REGION_SHAPE.RECT:
      ...
      var preserve_aspect_ratio = true;
...
window.addEventListener('keydown', function(e) {
  ...
  if (e.which === 112) { // F1 for help
    /* show_getting_started_panel(); @Ira*/
    e.preventDefault();
    return;
  }
...
function change_class_to(sel_class_name) {
	if(_via_is_region_selected){
		console.log(_via_user_sel_region_id);
		var region_id = _via_user_sel_region_id;
		_via_img_metadata[_via_image_id].regions[region_id].region_attributes["classLabel"] = sel_class_name;
		update_region_attributes_input_panel();
	}
	if (_via_is_reg_attr_panel_visible) {
		set_region_select_state(region_id, false);
